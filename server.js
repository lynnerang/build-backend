...
app.get('/api/v1/stars', (req, res) => {
	database('stars')
		.select()
		.then(stars => {
			stars.length ? res.status(200).json(stars) : res.status(404).json({ error: 'No star data exists.' });
		})
		.catch(error => res.status(500).json({ error }));
});
...
app.get('/api/v1/stars/:id', (req, res) => {
	...
	database('stars')
		...
		.select()
		.then(stars => {
			stars.length ? res.status(200).json(stars) : res.status(404).json({ error: 'No star data exists for that id.' });
		})
		.catch(error => res.status(500).json({ error }));
});
